...
const UI_MAIN_MENU := "MainMenuUI"

var _fsm = preload("res://Scripts/Utils/SimpleStateMachine.gd").new()
var _state_from: GameState = GameState.BOOT
...
func _enter_main_menu() -> void:
	...
	if ui != null:
		ui.enqueue_open_ui(UI_MAIN_MENU)
	var scene: SceneService = Game.Instance.try_get_service(Game.SERVICE_SCENE)
	var should_play_transition := _state_from != GameState.BOOT
	scene.change_scene.call_deferred(MAIN_MENU_SCENE_PATH, Callable(), should_play_transition)
...
func _enter_game() -> void: 
	...
	ui_service.enqueue_open_ui("HUDUI")
